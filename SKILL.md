...
3. 修正は以下の順序で行う:
   ...
   **ステップ1: Node.js バージョンの解決（最優先）**
   - チェック1-A を先に完結させる
   - Node.js 22未満の場合は `.node-version` を更新してからステップ2に進む
   - 22以上がローカルに存在しない場合はここで中断し、ユーザーにインストールを依頼する

   **ステップ2: pnpm 関連の修正**
   - チェック1-B〜E を順に対応する
   - 明らかな機械的修正（npmをpnpmに変える等）は確認なしで進めてよい
   - workspace 構成の追加・ロックファイルの整理を行った後、最後に以下を実行する（ユーザーの権限許可が必要）:
     1. `rm package-lock.json`（存在する場合のみ）
     2. `pnpm install`
   - `pnpm install` がビルドスクリプトの実行許可エラーで失敗した場合、`pnpm-workspace.yaml` の `allowBuilds` フィールドに該当パッケージを追記して対処する。**`package.json` の `pnpm` フィールドへの記述は行わないこと**（最新の pnpm では `pnpm-workspace.yaml` での管理が推奨されている）

   **ステップ3: その他の修正**
   - チェック2〜4（Firebase認証・firebase-admin・.babelrc）を対応する
